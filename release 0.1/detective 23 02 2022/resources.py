...
def split_spritesheet(filename, size, cols, rows, colorkey = None, scaleimg = None, alpha = False):
    ...
    for y in range(rows):
        for x in range(cols):
            ...
            subsurfaces[int(x+y*cols)] = tile # int(y+x*rows)
...
class LoadAssets(object):
    def __init__(self):
        ...
        sheet1 = split_spritesheet(resource_path('assets/democart.png'), (8, 8), 16, 16)
        # игрок и его анимации
        sheet2 = split_spritesheet(resource_path('assets/neato_16_8.png'), (8, 8), 16, 16)
        # детали графики уровня, яблоко, сердце
        sheet3 = split_spritesheet(resource_path('assets/tbj2_startcart2spr.png'), (8, 8), 16, 16)
        # курица, жук, человечек, улитка
        sheet4 = split_spritesheet(resource_path('assets/add_sprites.png'), (8, 8), 16, 16)
        # символы PICO-8
        sheet5 = split_spritesheet(resource_path('assets/pico8_symbols.png'), (8, 8), 16, 2, pygame.Color('black'))
        ...
        for fname in os.listdir(resource_path('parts')):
            ...
            if img_type == 'facial_hair':
                ...
                img = pygame.Surface((tw*8, th*8)).convert()
        ...
        self.case_main_prompts = []
        self.case_false_prompts = []
        self.true_criminal = []
        ...
        self.talks = {'boss': {'new_clues':['TODAY WE\'VE GOT A MYSTERIOUS {} BURGLARY IN OUR LOCAL MUSEUM.FIND THAT CRIMINAL,I WANT TO LOOK AT HIS EYES!',
                                            'SOMEONE IN THIS TOWN DID AN DISGUSTING ATROCITY.{} HAS BEEN STOLEN IN THE MIDDLE OF BAZZAR!',
                               ...
                               'end':['IT IS ENOUGH FOR YOU THIS DAY,DETECTIVE.YOU CANNOT EARN ALL BUCKS IN THE WORLD.GO HOME AND REST NOW'],
                     ...
                     'witness': {'add_clue':['I SAW THIS {}{}{} SOMWHERE IN TOWN.MAYBE IT WILL HELP YOU',
                                            'THIS {}{}{} WERE LYING ON THE GROUND NEAR CRIME SCENE.YOU MIGHT KNOW WHAT IS THAT',
                                            'TAKE {}{}{} UNDER THE CUSTODY,PLEASE!I\'M TOO SCARED TO HAVE IT AT MY PLACE'],

                                 'chat': ['I ALREADY TOLD YOU ALL WHAT I KNOW.LEAVE ME ALONE,PLEASE'],
                    ...
                    'suspect': {'name': 'SUSPECT',
                                'chat': ['I WILL TELL YOU NOTHING,COP!I HAVE A RIGHT TO MAKE A ONE CALL!',
                    ...
                    'janitor':{'name': 'JEREMIAH',
                               'chat': ['PLEASE,DON\'T STEP ON PUDDLES...YOU MAY FALL',
                                        'PLEASE,DON\'T SLAM OUR DOORS.IF YOU BRAKE IT,IT WILL HURT YOUR PAY-CHECKS',
                                        'TIME IS ALL THAT MATTERS,AFTER ALL...OH,LUNCH TIME!',
                                        'SOME PEOPLE SAYS THAT IF YOU WILL BE TRYING HARD ENOUGH,YOU COULD BEAT ANYTHING.LIARS!',
                   ...
                   'archivist':{'name': 'JOE',
                               'chat': ['WELL HELLO THERE!MAY I ASK WHAT\'S ON YOUT MIND?I THINK,I CAN HELP',
                                        ...
                                        'COMPUTERS ARE AMAZING AT STORING INFORMATION,BUT PERSANALLY I PREFER REAL PAPER',
                                        'PLEASE, DON\'T USE ANY ILLEAGAL SOFTWARE.I TIRED OF CLEANING AFTER YOU,SMART-GUYS',
                   ...
                   'cop1':{'name': 'TERRY',
                           'chat': ['HOW IS YOUR DAY GOING?',
                                    'TRY NOT TO LISTEN TO OUR LAYER RAMBLINGS.HE GOT PAID ON EVERY WORD HE SPITS',
                   ...
                   'layer':{'name': 'MR.TYLER',
                            'chat': ['HI!<HE IS WATCHING THROUGH YOU, IT LOOKS LIKE MANY THOUGHTS WHIRLING IN HIS HEAD>',
                                     ...
                                     'MATTHEW AND TYLOR ARE GRATE GUYS,BUT A LITTE GRIM',
                                     'YOURS JANITOR IS DOING AMAZING JOB HERE!FLOOR IS SO SMOOTH AND CLEAN THAT I ALMOST FALL ONCE!HA-HA!',
                                     ...
                                     'I WISH SOME DAY LAW WILL BE HANDLED BY COLD BLOODED ROBOT THAT WILL BE DECIDE BASED ON FACTS AND NOTHING ELSE MORE'],
                   ...
                   'dutyofficer':{'name': 'ROB',
                                  ...
                                  'chat':['HEY,YOU!FOR WHAT PURPOSE DID YOU CO...A,HELLO,DETECTIVE',
                                          ...
                                          'YOU LOOKS A BIT PALE,IS EVERYTHING ALRIGHT?',
                                          ...
                                          'I DON\'T LIKE LAYER-GUY.TYLER,ACCORDING TO PAPERS.THAT SNAKE DEFFENDING CRIMINALS!',
        ...
        self.thoughts = {'start1': 'NEW DAY,NEW JOY...',
                         ...
                         '1st_ikit_send_out': 'NOW LET\'S TAKE A BRAKE.TIME TO GET SOME COFFEE AND THEN CHECK JAIL',
        ...
        todolist_text = ['TRY TO SEARCH IN ARCHIVE FOR NEW CLUES',
                         'ASK WITNESESS,IF THEY WILL COME TO THE STATION',
        ...
        self.titles_surf = pygame.Surface((TS*15, surf_height+TS*15))
        ...
        self.labels = {'quit':None, 'try':None, 'make':None, 'no':None, 'yes':None, 'got_it':None,
                       ...
                       '1':None, '2':None,
                       'interr':None, 'case':None, 'mytodo':None, 'caseN':None,'susp':None, 'ikit':None}
        label_text = ['ÎQUIT','TRY×','MAKE×','ÎNO','YES×','GOT IT×',
                      'Ë', 'Ñ', 'Ô', 'Ã', '1', '2',
                      'INTERROGATION:', 'CASE:', 'MY TODO LIST:', 'CASE #', 'SUSPECT','IDENTIKIT:']
        ...
        # Название игры в Start
        s_text = ['TOYBOX', 'DETECTIVE', ' NAVIGATE', ' X CONFIRM', ' Z DENY', 'ESC PAUSE']
        self.start_labels = []
        for s in s_text:
            t = text_mono.render_text(s, self.tile_font, 1)
        ...
        texts = {'BossBriefroom': ['×>TALK TO BOSS'],
                 ...
                 'Jail': ['×>GO TO CORRIDOR', '×>CLIMB UP', '×>GO DOWN', '×>TALK TO LAYER', '×>INTERROGATE SUSPECT'],
        ...
        for room_key in list(texts.keys()):
            room_helpers = []
            for t in texts[room_key]:
                room_helpers.append(text_mono.render_text(t, self.tile_font, 1))
            self.text_helpers[room_key] = room_helpers.copy()


        d = pygame.Surface((8, 8*2)).convert()
        d.blit(sheet1[88], (0, 0))
        d.blit(sheet1[88], (0, 8))
        self.closing_door_img = d.copy()

        # # Стрелки вверх/вниз для скролла текста в DialogueBox
        # self.dialogue_arrows = [sheet1[127].copy(), pygame.transform.flip(sheet1[127], False, True)] # U, D
        # # Синяя точка для отметки выбранной части фоторобота
        # self.dots = {'blue_dot': [sheet1[77].copy(), sheet1[78].copy()], 'locked':sheet1[74].copy()}
        ...
        arrow = pygame.Surface((8, 8*2)).convert()
        arrow.blit(sheet1[119], (0, 0))
        arrow.blit(pygame.transform.flip(sheet1[119], False, True), (0, 8))
        ...
        # Ответы в диалоге Interrogation
        self.interr_answer_imgs = {'yes':text_mono.render_text('YES', self.tile_font, 1), 'no':text_mono.render_text('NO!', self.tile_font, 1),
                                   'hmmm':text_mono.render_text('HMMM...', self.tile_font, 1)}
        ...
        self.coin_locs = []
        for i in range(16):
            self.coin_locs.append((i*8, -8))
        print(self.coin_locs)
    ...
    def clue_to_text(self, clue):
        clues = {'sword':'a','pickaxe':'b','hammer':'c','chest':'d','g_key':'e','s_key':'f','bottle':'g','r_heart':'h',
                 'gun':'i','goblet':'j','gem':'k','grave':'l','bell':'m','pike':'n','skull':'o','bubble':'p','leaf':'q',
                 'coin':'r','o_lock':'s','g_apple':'t','g_heart':'u'}
        return clues[clue]
    ...
    def make_cheat_img(self):
        # Картинка с ЧИТОМ
        self.cheat_image = pygame.Surface((128, 136)).convert()
        self.cheat_image.set_colorkey(pygame.Color('magenta'))
        self.cheat_image.fill(pygame.Color('magenta'))

        pygame.draw.rect(self.cheat_image, pygame.Color('black'), (0, 0, 8*16, 8*17))

        # Части ФР
        u_cheat = self.identikit_parts['upper'][self.true_criminal[0]['ident_part_num']]
        self.cheat_image.blit(u_cheat, (8*5, 8*3))

        l_cheat = self.identikit_parts['lower'][self.true_criminal[1]['ident_part_num']]
        self.cheat_image.blit(l_cheat, (8*5, 8*7))

        i_p_num = self.true_criminal[2]['ident_part_num']
        if i_p_num == self.bald_parts['hair'][0]:
            h_cheat = self.bald_parts['hair'][1]
        else:
            h_cheat = self.identikit_parts['hair'][self.true_criminal[2]['ident_part_num']]
        self.cheat_image.blit(h_cheat, (8*5, 8*3))

        i_p_num = self.true_criminal[3]['ident_part_num']
        if i_p_num == self.bald_parts['facial_hair'][0]:
            fh_cheat = self.bald_parts['facial_hair'][1]
        else:
            fh_cheat = self.identikit_parts['facial_hair'][self.true_criminal[3]['ident_part_num']]
        self.cheat_image.blit(fh_cheat, (8*5, 8*8))

        # Иконки
        for i in range(4):
            clue_icon = self.clues_icons[self.case_main_clues[i]]
            self.cheat_image.blit(clue_icon, (0, 8*i))

        for i in range(4):
            for j in range(3):
                clue_icon = self.clues_icons[self.case_main_prompts[i][j]]
                self.cheat_image.blit(clue_icon, (8*2+j*8, 8*i))

        for i in range(4):
            part_str = ''
            for tp in self.true_criminal:
                if tp['assigned_prompt'][0] == self.case_main_clues[i]:
                    if tp['ident_part'] == 'upper':
                        part_str = 'U'
                    elif tp['ident_part'] == 'lower':
                        part_str = 'L'
                    elif tp['ident_part'] == 'hair':
                        part_str = 'H'
                    elif tp['ident_part'] == 'facial_hair':
                        part_str = 'FH'
                    break
            self.cheat_image.blit(text_mono.render_text(part_str, self.tile_font, 1), (8*6, 8*i))
    ...
    def generate_case(self):
        ...
        self.case_main_prompts = [] # [[str, ..., ...], [str, ...], ...]
        for p in self.case_main_clues:
            ...
            for cl in add_clues:
                prompt.append(cl)
                clues.remove(cl)
            self.case_main_prompts.append(prompt)
        print('\tMain prompts:', self.case_main_prompts)

        self.case_false_prompts = [] # [[str, ..., ...], [str, ...], ...]
        ...
        for i in range(false_count):
            self.case_false_prompts.append(random.sample(clues_no_main, 3))
        print('\tFalse prompts:\n\t', self.case_false_prompts, len(self.case_false_prompts))

        mp_idxs = [0, 1, 2, 3]
        random.shuffle(mp_idxs)
        self.true_criminal = [{'ident_part':'upper', 'ident_part_num':0, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_main_prompts[mp_idxs[0]][:],'mp_idx':mp_idxs[0]}, #'TRUEUPPER'
                              {'ident_part':'lower', 'ident_part_num':0, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_main_prompts[mp_idxs[1]][:],'mp_idx':mp_idxs[1]}, #'TRUELOWER'
                              {'ident_part':'hair', 'ident_part_num':0, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_main_prompts[mp_idxs[2]][:],'mp_idx':mp_idxs[2]}, #'TRUEHAIR '
                              {'ident_part':'facial_hair', 'ident_part_num':0, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_main_prompts[mp_idxs[3]].copy(),'mp_idx':mp_idxs[3]}] #'TRUEFACHA'
        ...
        random.shuffle(upp)
        for i in upp:
            newcase = {'ident_part':'upper', 'ident_part_num':i, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_false_prompts[n]} # 'FALSUPP'+str(n)
        ...
        random.shuffle(low)
        for i in low:
            newcase = {'ident_part':'lower', 'ident_part_num':i, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_false_prompts[n]}
        ...
        random.shuffle(hai)
        for i in hai:
            newcase = {'ident_part':'hair', 'ident_part_num':i, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_false_prompts[n]}
        ...
        random.shuffle(fha)
        for i in fha:
            newcase = {'ident_part':'facial_hair', 'ident_part_num':i, 'case':self.get_rnd_case_name(), 'assigned_prompt':self.case_false_prompts[n]}
