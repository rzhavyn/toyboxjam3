...
class Coin(Actor):
    ...
    def __init__(self, parent, pos):
        ...
        self.image = anim_set_frames[0] # random.randint(0, 1)
        self.pos = pos
...
class Player(Actor):
    ...
    def update(self, delta):
        ...
        # Апдейт картинки согласно текущей анимации и ее кадру
        # Климб не имеет направления лево-право
...
class NPC(Actor):
    ...
    def __init__(self, parent, pos, npc_type, npc_scene_name, talk_rect, ikit_parts=None, wander_point=None, wander_delay=0):
        self._layer = 2
        ...
        # random.shuffle(self.animation_set['anim_frames'][self.state.value]) # анимация IDLE начнется с разных случайных кадров
        ...
        # self.angry = False # доп. анимация фонтана звездочек и подпрыгивания на месте
    ...
    def update(self, delta):
        ...
        if player.rect.centerx > self.talk_rect.left and player.rect.centerx < self.talk_rect.right:
            if player.rect.centery > self.talk_rect.top and player.rect.centery < self.talk_rect.bottom:
                ...
                # Стартовая сцена, авто разговор DO с игроком
...
class Indicator(pygame.sprite.Sprite):
    ...
    def __init__(self, parent):
        ...
        self.transp_image = pygame.Surface((8, 8)).convert()
        ...
        if self.opens_new_scene:
            if self.parent.fade.finished_out:
                if self.active:
                    args_dict = {'player_pos': Vector2(player.rect.topleft),
                                 'player_facing': player.facing,
                                 'identikit':{'upper':2,'lower':1,'hair':4,'facial_hair':3}} # TEST
...
# case = {'ident_part': 'upper', 'ident_part_num': 3, 'case': str, 'assigned_prompt': ['prompt1','prompt2','prompt3']}
...
class Identikit(object):
    ...
    def __init__(self, parent, parts_locs, is_changeble=True, preview_kit=True):
        ...
        self.cases = []
        print('__init__ IKIT CASES:', self.cases)
        self.case_idx = 0
    ...
    def get_parts_numbers(self):
        out = {'upper':self.upper_part.ident_part_num, 'lower':self.lower_part.ident_part_num,
               'hair':self.hair_part.ident_part_num, 'facial_hair':self.facial_hair_part.ident_part_num}
        return out


    def change_parts(self):
        print('update IKIT CASES:', self.cases)
        ...
        if cur_case['ident_part'] == 'upper':
            self.upper_part.change(cur_case['ident_part_num'])
            self.lower_part.change(0)
            self.hair_part.change(0)
            self.facial_hair_part.change(0)
        elif cur_case['ident_part'] == 'lower':
            self.upper_part.change(0)
            self.lower_part.change(cur_case['ident_part_num'])
            self.hair_part.change(0)
            self.facial_hair_part.change(0)
        elif cur_case['ident_part'] == 'hair':
            self.upper_part.change(0)
            self.lower_part.change(0)
            self.hair_part.change(cur_case['ident_part_num'])
            self.facial_hair_part.change(0)
        elif cur_case['ident_part'] == 'facial_hair':
            self.upper_part.change(0)
            self.lower_part.change(0)
            self.hair_part.change(0)
            self.facial_hair_part.change(cur_case['ident_part_num'])
    ...
    def update(self):
        if self.is_changeble:
            if self.key_control.left:
                if not self.once:
                    if DEBUG: print('IDENTIKIT CYCLING <')
                    self.once = True
                    self.key_control.clear_keys()
...
class IdentikitPart(pygame.sprite.Sprite):
    ...
    def __init__(self, parent, pos, ident_part):
        self._layer = 0
        ...
        self.ident_part = ident_part # 'upper', 'lower', 'hair', 'facial_hair'
        self.ident_part_num = 0 # ! 0 элемент всегда прозрачная пустая картинка

        self.image = self.assets.identikit_parts[self.ident_part][self.ident_part_num]
        self.pos = pos
        self.rect = self.image.get_rect(topleft = self.pos)
    ...
    def change(self, ident_part_num):
        self.ident_part_num = ident_part_num

        if not self.parent.preview_kit:
            if self.ident_part == 'hair':
                if ident_part_num == self.assets.bald_parts['hair'][0]:
                    self.bald_part = True
                else:
                    self.bald_part = False

            elif self.ident_part == 'facial_hair':
                if ident_part_num == self.assets.bald_parts['facial_hair'][0]:
                    self.bald_part = True
                else:
                    self.bald_part = False
    ...
    def update(self):
        if self.bald_part:
            self.image = self.assets.bald_parts[self.ident_part][1]
        else:
            self.image = self.assets.identikit_parts[self.ident_part][self.ident_part_num]
...
class UIPrompt(object):
    ...
    def set_prompt(self, prompts_list): #[['s', 's', 's'], [], ....]
        if DEBUG: print('SET PROMPT:', prompts_list)
        self.prompts_images = []

        for i in range(len(prompts_list)):
            if prompts_list[i] != []:
                for j in range(len(prompts_list[i])):
                    self.def_image.blit(self.assets.clues_icons[prompts_list[i][j]], (TS*j, 0))
            else:
                self.def_image.fill(pygame.Color('black'))

            self.prompts_images.append(self.def_image.copy())


    def update(self):
        if self.parent.identikit.case_idx != self.archive_case_n:
            self.set_prompt([self.parent.identikit.cases[self.parent.identikit.case_idx]['assigned_prompt']])
            self.archive_case_n = self.parent.identikit.case_idx


    def render(self, surface):
        for i, img in enumerate(self.prompts_images):
...
class UIAnswer(pygame.sprite.Sprite):
    ...
    def __init__(self, parent, pos):
        ...
        self.images = parent.assets.interr_answer_imgs
    ...
    def set_image(self, answer):
        if answer == 'yes':
            self.image = self.images['yes']
        elif answer == 'no':
            self.image = self.images['no']
        elif answer == 'hmmm':
            self.image = self.images['hmmm']
        else:
            self.image = self.no_image
...
class DialogueBox(object):
    ...
    def __init__(self, parent, pos):
        ...
        # self.arrows = self.assets.dialogue_arrows
    ...
    def show_talks(self, talk_to_actor, talk_type):
        ...
        if self.conv_npc.npc_scene_name == 'boss':
            ...
            if talk_type == 'add_clue':
                ...
                a_prompts = self.conv_npc.ikit_parts['assigned_prompt']
                text_to_render = phrase.format(self.assets.clue_to_text(a_prompts[0]), self.assets.clue_to_text(a_prompts[1]),
                                               self.assets.clue_to_text(a_prompts[2]))
        ...
        self.name_image = text_mono.render_text(name_to_render, self.assets.tile_font, 1)
            ...
            if self.key_control.x:
                ...
                if self.conv_npc.npc_scene_name == 'witness':
                    if not self.conv_npc.refuse_to_talk:
                        ...
                        clue = self.conv_npc.ikit_parts['assigned_prompt'][0]
                        clue_part = self.conv_npc.ikit_parts['ident_part']
                        if clue not in self.director.investigated_clues:
                             self.director.investigated_clues.append(clue)
                             # self.director.investigated_main_prompts[clue_part] = self.conv_npc.ikit_parts # интереснее, когда не добавляется подсказка напрямую
                        else:
                            if self.director.investigated_main_prompts[clue_part] == None:
                                self.director.investigated_main_prompts[clue_part] = self.conv_npc.ikit_parts
                        print('Dbox:')
