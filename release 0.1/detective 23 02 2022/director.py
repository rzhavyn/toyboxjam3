...
class Director(object):
    ...
    def __init__(self):
        ...
        self.investigated_main_prompts = {'upper':None,'lower':None,'hair':None,'facial_hair':None} # правильные подсказки/части ФР
        self.all_clues_investigated = False
        self.all_prompts_investigated = False
    ...
    def refresh_npcs_on_new_ikit(self):
        ...
        for npc_key in list(self.curr_ikit_npcs.keys()):
            if npc_key == 'false_suspects':
                ...
                for npc in gen_npcs:
                    suspect = NPC(jail_scene, npc['loc'], npc['npc_type'], 'suspect', npc['talk_rect'], npc['ikit_parts_cases'], npc['wander_p'], 35)
