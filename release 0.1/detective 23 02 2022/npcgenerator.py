...
class NPCGenerator(object):
    ...
    def __init__(self, parent):
        ...
        self.def_suspect = {'npc_type': None, 'loc': None, 'wander_p': None, 'talk_rect': None, 'ikit_parts_cases':[]} # 'arch_cases'
            ...
            if false_ikit_cases == []:
                ...
                if len(false_ikit_cases) <= 2:
                    ...
                    for j in range(n_false):
                        ...
                        if rnd_ic['ident_part'] == 'upper':
                            out[0] = rnd_ic
                        elif rnd_ic['ident_part'] == 'lower':
                            out[1] = rnd_ic
                        elif rnd_ic['ident_part'] == 'hair':
                            out[2] = rnd_ic
                        elif rnd_ic['ident_part'] == 'facial_hair':
                            out[3] = rnd_ic
    ...
    def gen_witness(self):
        ...
        used_iparts = []
        for p in self.used:
            used_iparts.append(p['ident_part'])

        left_unused = []
        for tc in self.assets.true_criminal:
            if tc['ident_part'] not in used_iparts:
        ...
        if left_unused != []:
            ...
            if DEBUG:print('\t\tNo free main prompts for witness')
            return []
    ...
    def gen_true_suspects(self, true_ikit_cases, false_ikit_cases):
        ...
        if not self.parent.all_prompts_investigated:
            add_prompt_idx = random.randint(0, len(true_ikit_cases)-1)
            if DEBUG: print('\tSuspect idx to add prompt:', add_prompt_idx)
            ...
            for i in range(len(true_ikit_cases)):
                ...
                if i == add_prompt_idx:
                    ...
                    if not true_ikit_part['assigned_prompt'][0] in self.parent.investigated_clues:
                        print('\t\ttrue_ikit_part main clue {} NOT in investigated_clues {}'.format(true_ikit_part['assigned_prompt'][0], self.parent.investigated_clues))
                        # вгенеировать true_ikit_part
                        if true_ikit_part['ident_part'] == 'upper':
                            out[0] = true_ikit_part
                        elif true_ikit_part['ident_part'] == 'lower':
                            out[1] = true_ikit_part
                        elif true_ikit_part['ident_part'] == 'hair':
                            out[2] = true_ikit_part
                        elif true_ikit_part['ident_part'] == 'facial_hair':
                            out[3] = true_ikit_part
                        # добавть в использованные в генерации
                        self.used.append(true_ikit_part)

                        # + вгенерировать доп true_cur_part по одной зацепке, если ее нет в investigated_main_prompts:
                        # найти зацепку, у которой нет prompt
                        clues_without_prompt = []
                        for c in self.parent.investigated_clues:
                            for p in list(self.parent.investigated_main_prompts.values()):
                                if p != None:
                                    if p['assigned_prompt'][0] != c:
                                        if c not in clues_without_prompt:
                                            clues_without_prompt.append(c)
                                else:
                                    if c not in clues_without_prompt:
                                        clues_without_prompt.append(c)

                        print('\t\tclues_without_prompt {}'.format(clues_without_prompt))

                        clue_part = None
                        # найти часть true_criminal, соотв этой зацепке
                        if clues_without_prompt != []:
                            for tp in self.assets.true_criminal:
                                if tp['assigned_prompt'][0] == clues_without_prompt[0]:
                                    clue_part = tp
                                    break

                            print('\t\t+ clue_part to suspect: {}'.format(clue_part))

                            if clue_part['ident_part'] == 'upper':
                                out[0] = clue_part
                            elif clue_part['ident_part'] == 'lower':
                                out[1] = clue_part
                            elif clue_part['ident_part'] == 'hair':
                                out[2] = clue_part
                            elif clue_part['ident_part'] == 'facial_hair':
                                out[3] = clue_part
                            # добавть в использованные в генерации
                            self.used.append(clue_part)

                    else:
                        print('\t\ttrue_ikit_part main clue {} IN investigated_clues {}'.format(true_ikit_part['assigned_prompt'][0], self.parent.investigated_clues))
                        # проверить есть ли зацепка в investigated_main_prompts
                        # найти зацепку, у которой нет prompt
                        clue_in_main_prompts = False
                        for p in list(self.parent.investigated_main_prompts.values()):
                            if p != None:
                                if p['assigned_prompt'][0] == true_ikit_part['assigned_prompt'][0]:
                                    clue_in_main_prompts = True
                                    break
                        # если нет:
                        if not clue_in_main_prompts:
                            # вгенеировать true_ikit_part
                            if true_ikit_part['ident_part'] == 'upper':
                                out[0] = true_ikit_part
                            elif true_ikit_part['ident_part'] == 'lower':
                        ...
                        # если есть:
                            ...
                            # # Добавить случаную ложную часть
                            # if true_ikit_part['ident_part'] != 'upper':
                            #     out[0] = self.get_rnd_ip('upper', self.upper_used, self.assets.false_suspects_parts)
                            # if true_ikit_part['ident_part'] != 'lower':
                            #     out[1] = self.get_rnd_ip('lower', self.lower_used, self.assets.false_suspects_parts)
                            # if true_ikit_part['ident_part'] != 'hair':
                            #     out[2] = self.get_rnd_ip('hair', self.hair_used, self.assets.false_suspects_parts)
                            # if true_ikit_part['ident_part'] != 'facial_hair':
                            #     out[3] = self.get_rnd_ip('facial_hair', self.facial_hair_used, self.assets.false_suspects_parts)

                            # # Добавить вместо нее дополнительную подсказку!
                            # # найти зацепку в case_main_prompts, которой нет в investigated_clues
                            # not_inv_main_clues = []
                            # for mc in self.assets.case_main_clues:
                            #     if mc not in self.parent.investigated_clues:
                            #         not_inv_main_clues.append(mc)
                            #
                            # print('\t\tnot investigated main clues: {}'.format(not_inv_main_clues))
                            #
                            # clue_part = None
                            # # найти часть true_criminal, соотв этой зацепке (на 0 позиции)
                            # if not_inv_main_clues != []:
                            #     for tp in self.assets.true_criminal:
                            #         if tp['assigned_prompt'][0] == not_inv_main_clues[0]:
                            #             clue_part = tp
                            #             break
                            #
                            #     print('\t\t+ clue_part to suspect: {}'.format(clue_part))
                            ...
                            #     self.used.append(true_ikit_part)
                            # else:
                            #     print('\t\tall main clues already investigated')
                ...
                print('\t\tUSED main prompts:', self.used)
                print('\t\tout:', out)

                if false_ikit_cases == []:
                    # Остальные случайные
                    if true_ikit_cases[i]['ident_part'] != 'upper':
                        out[0] = self.get_rnd_ip('upper', self.upper_used, self.assets.false_suspects_parts)
                    if true_ikit_cases[i]['ident_part'] != 'lower':
                        out[1] = self.get_rnd_ip('lower', self.lower_used, self.assets.false_suspects_parts)
                    if true_ikit_cases[i]['ident_part'] != 'hair':
                        out[2] = self.get_rnd_ip('hair', self.hair_used, self.assets.false_suspects_parts)
                    if true_ikit_cases[i]['ident_part'] != 'facial_hair':
                        out[3] = self.get_rnd_ip('facial_hair', self.facial_hair_used, self.assets.false_suspects_parts)
                else:
                    # 1 или 2 части берутся из false_ikit_cases
                    if len(false_ikit_cases) == 1:
                    ...
                    used_ips = []
                    for up in self.used:
                        used_ips.append(up['ident_part'])

                    for j in range(n_false):
                        for k in range(len(false_ikit_cases_copy)):
                            rnd_ic = random.choice(false_ikit_cases_copy)
                            if rnd_ic['ident_part'] in used_ips:
                                false_ikit_cases_copy.remove(rnd_ic)
    ...
    def gen_current_ikit_npcs(self, ikit):
        ...
        if ikit['upper'] == self.assets.true_crim_ip_num[0]:
            ...
            for fp in self.assets.false_suspects_parts['upper']:
                if fp['ident_part_num'] == ikit['upper']:
        ...
        if ikit['lower'] == self.assets.true_crim_ip_num[1]:
            ...
            for fp in self.assets.false_suspects_parts['lower']:
                if fp['ident_part_num'] == ikit['lower']:
                    false_ikit_cases.append(fp)
                    break
        ...
        if ikit['hair'] == self.assets.true_crim_ip_num[2]:
            ...
            for fp in self.assets.false_suspects_parts['hair']:
                if fp['ident_part_num'] == ikit['hair']:
                    false_ikit_cases.append(fp)
                    break
        ...
        if ikit['facial_hair'] == self.assets.true_crim_ip_num[3]:
            ...
            for fp in self.assets.false_suspects_parts['facial_hair']:
                if fp['ident_part_num'] == ikit['facial_hair']:
                    false_ikit_cases.append(fp)
                    break
